...
def read_sales_report(start_date, end_date):
    ...
    query = """
        ...
        s.data_venda, 
        s.total 
        + COALESCE(i.total_ifood, 0) 
        + COALESCE(N.total_99food, 0)
        + COALESCE(k.total_keeta, 0) AS total, 
        ...
        COALESCE(k.total_keeta, 0) AS total_keeta,
        COALESCE(k.qtd_keeta, 0) AS qtd_vendas_keeta,
        ROUND(COALESCE(k.ticket_medio_keeta, 0), 2) AS ticket_medio_keeta
    FROM (
        SELECT 
    ...
    LEFT JOIN (
        SELECT 
            DATE(od.order_date) AS data_venda,
            SUM(od.item_value) AS total_keeta,
            COUNT(1) AS qtd_keeta,
            SUM(od.item_value) / COUNT(1) AS ticket_medio_keeta
        FROM order_delivery od
        WHERE 1=1
        AND LOWER(od.sales_channel) IN ('keeta')
        AND od.cancellation_source IS NULL
        GROUP BY DATE(od.order_date)
    ) AS k ON k.data_venda = s.data_venda
    ...
    column_rename_map = {
        ...
        'ticket_medio_99food': 'Ticket Médio 99food',
            # --- KEETA ---
        'total_keeta': 'Total Keeta',
        'qtd_vendas_keeta': 'Qtd. Vendas Keeta',
        'ticket_medio_keeta': 'Ticket Médio Keeta',
    }
    with get_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if not df.empty:
                ...
                total_row_data['qtd_vendas_99food'] +
                total_row_data['qtd_vendas_keeta']
            )
                ...
                total_row_data['ticket_medio_keeta'] = total_row_data['total_keeta'] / total_row_data['qtd_vendas_keeta'] if total_row_data['qtd_vendas_keeta'] > 0 else 0
                ...
                cols_inteiras = ['qtd_vendas_loja', 'qtd_vendas_ifood', 'qtd_vendas_99food', 'qtd_vendas_keeta'] # Adicionado 99food
